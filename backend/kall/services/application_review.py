...
def build_review(session: Session, application: Application) -> ApplicationReview:
    ...
    for payload in detect_questions(application):
    ...
    session.add(review)
    session.add(ApplicationReviewAudit(
        application_id=application.id,
        user_id=application.user_id,
        event="review_created",
        details={"question_count": len(detect_questions(application))},
    ))
    session.commit()
...
def approve_review(session: Session, application: Application, review: ApplicationReview) -> ApplicationReview:
    issues = calculate_readiness(session, application, review)
    if issues:
    ...
    session.add(ApplicationReviewAudit(
        ...
        details={"submission_allowed": False},
    ))
    session.commit()
    session.refresh(review)
    return review
