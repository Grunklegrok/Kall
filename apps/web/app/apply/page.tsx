...
export default function ApplyPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (resumeResponse.status === 401 || profileResponse.status === 401) {
      ...
      const preferredResume = loadedResumes.find((item: Resume) => item.id === firstProfile?.default_resume_id)
        || loadedResumes.find((item: Resume) => item.is_default)
        || loadedResumes[0];
  ...
  async function prepare() {
    ...
    try {
      ...
      setMessage('Application prepared. Review and explicit approval are required before submission.');
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Unable to prepare this application.');
    } finally {
      setPreparing(false);
    }
  }
  ...
  return (
    <main className="shell">
      <header className="topbar">
        <a className="brand" href="/">Kall Apply</a>
        <nav><a href="/opportunities">Opportunities</a><a href="/resumes">Resume Studio</a></nav>
      </header>

      <section className="hero" style={{ paddingBottom: 36 }}>
        <span className="eyebrow">Application preparation</span>
        <h1>Choose how Kall should prepare this application.</h1>
        <p>Kall can pair the role with a stored profile and resume, draft tailored documents, and prepare an assisted or automatic application workflow for your review.</p>
      </section>

      <div className="application-prep-columns">
        <section className="card">
          <span className="pill">Selected role</span>
          <h2 style={{ marginTop: 16 }}>{externalTitle}</h2>
          <p>{externalSnippet || 'The complete posting will remain available through the original job link.'}</p>
          {externalUrl && <a className="button secondary" href={externalUrl} target="_blank" rel="noreferrer" style={{ marginTop: 18 }}>View original posting</a>}
        </section>
        ...
        <section className="card form">
          ...
          <fieldset className="application-options">
            <legend>AI document preparation</legend>
            <label className="check-row"><input type="checkbox" checked={customizeResume} onChange={(event) => setCustomizeResume(event.target.checked)} />Customize the selected resume for this role</label>
            <label className="check-row"><input type="checkbox" checked={generateCoverLetter} onChange={(event) => setGenerateCoverLetter(event.target.checked)} />Generate a role-specific cover letter draft</label>
          </fieldset>

          <fieldset className="application-options">
            <legend>Application mode</legend>
            <label className="check-row"><input type="radio" name="mode" checked={applicationMode === 'assisted'} onChange={() => setApplicationMode('assisted')} />Assisted — Kall prepares the package and guides me through the form</label>
            <label className="check-row"><input type="radio" name="mode" checked={applicationMode === 'automatic'} onChange={() => setApplicationMode('automatic')} />Automatic where supported — Kall prepares autofill data, then asks for final approval before submission</label>
          </fieldset>

          <button className="button" type="button" onClick={prepare} disabled={preparing || !profileId}>
            {preparing ? 'Preparing application…' : 'Prepare application'}
          </button>
      ...
      {application && (
        <section className="card" style={{ marginTop: 24 }}>
          ...
          <h2 style={{ marginTop: 16 }}>Review checklist</h2>
          <div className="grid">
            <article className="card"><h3>Resume</h3><p>{application.customized_resume_path || 'Original selected resume'}</p></article>
            <article className="card"><h3>Cover letter</h3><p>{application.cover_letter_path || 'Not requested'}</p></article>
            <article className="card"><h3>Submission</h3><p>Explicit review and approval are required before Kall submits or assists with submission.</p></article>
          </div>
          <p style={{ marginTop: 18 }}><strong>Open questions:</strong> {application.unanswered_questions.join(' · ') || 'None'}</p>
          <p style={{ marginTop: 10 }}><strong>Sensitive fields:</strong> {application.sensitive_fields_present ? 'Confirmation required' : 'None'}</p>
          <a className="button" href={`/applications?application=${application.id}`} style={{ marginTop: 20 }}>Continue to application review</a>
        </section>
      )}
    </main>
  );
